...
from regime_risk_engine.regimes.kmeans import KMeansRegimeConfig, KMeansRegimeModel
...
def test_kmeans_regime_model_fit_sets_model_state() -> None:
...
def test_kmeans_regime_model_predict_returns_labels() -> None:
    feature_matrix = make_feature_matrix()
    model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
...
def test_kmeans_regime_model_fit_predict_returns_labels() -> None:
    feature_matrix = make_feature_matrix()
    model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
...
def test_kmeans_regime_model_is_deterministic_with_fixed_seed() -> None:
    feature_matrix = make_feature_matrix()

    first_model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
    second_model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
...
def test_kmeans_regime_model_fit_predict_result() -> None:
    feature_matrix = make_feature_matrix()
    model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
    ...
    assert result.metadata["n_observations"] == 9
    assert len(result.labels) == len(feature_matrix)


def test_kmeans_regime_model_rejects_invalid_n_regimes() -> None:
    with pytest.raises(RegimeModelError, match="greater than 1"):
        KMeansRegimeModel(KMeansRegimeConfig(n_regimes=1))


def test_kmeans_regime_model_rejects_prediction_before_fit() -> None:
    feature_matrix = make_feature_matrix()
    model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
...
def test_kmeans_regime_model_rejects_missing_values() -> None:
    feature_matrix = make_feature_matrix()
...
def test_kmeans_regime_model_rejects_non_datetime_index() -> None:
    ...
    model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=2, random_state=42))

    with pytest.raises(RegimeModelError, match="DatetimeIndex"):
        model.fit(feature_matrix)


def test_kmeans_regime_model_rejects_changed_feature_columns() -> None:
    feature_matrix = make_feature_matrix()
    model = KMeansRegimeModel(KMeansRegimeConfig(n_regimes=3, random_state=42))
    model.fit(feature_matrix)
